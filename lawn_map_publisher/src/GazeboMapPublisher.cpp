//CODE IN THIS FILE TAKEN FROM 
//https://github.com/JenniferBuehler/gazebo-pkgs
//BY Jennifer Buehler 
//AND USED UNDER THE TERMS OF THE GPLv3 LICENSE
//SEE GPLv3.txt
//I HAVE FIXED SEVERAL BUGS BUT IT STILL HAS A LOT OF HARCODING FOR MY MAP - DJD
...
// // //      //   MSG_PRINT(" dX_horizontal=" << dX_horizontal << " dY_horizontal="<<dY_horizontal<<
...
//     MSG_PRINT("Rasterizing model and checking collisions");
    grid.resize(count_vertical,std::vector<int>(count_horizontal,blank));

    int steppp=count_vertical/20;
...
//           MSG_PRINT("Percent complete: " << i * 100.0 / count_vertical);
        ...
        for (int j = 0; j < count_horizontal; ++j) {
            ...
            double start_offsets_x[] = {dX_horizontal/2,-dX_horizontal/2,dX_vertical/2,-dX_vertical/2,0};
            double start_offsets_y[] = {dY_horizontal/2,-dY_horizontal/2,dY_vertical/2,-dY_vertical/2,0};
            double end_offsets_x[] = {-dX_horizontal/2,dX_horizontal/2,-dX_vertical/2,dX_vertical/2,0};
            double end_offsets_y[] = {-dY_horizontal/2,dY_horizontal/2,-dX_vertical/2,dY_vertical/2,0};

            bool obstcl=false;
            for(int kk=0;kk<5;kk++){

              start.x = x+start_offsets_x[kk];
              start.y = y+start_offsets_y[kk];;
              end.x = x+end_offsets_x[kk];
              end.y = y+end_offsets_y[kk];
  //             end.x = x;
  //             end.y = y;
...
//                       grid[i][j]=fill;
    ...
    meta.width=count_horizontal; //was probably wrong - bug
    meta.height=count_vertical; //was probably wrong - bug
    map.info=meta;
